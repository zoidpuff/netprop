...
l <- layout_with_fr(intGraph, niter=50)

# Start plot capture
png(paste0(netPropPath,"/figures/GraphPlot.png"), width = 2000, height = 200)
plot(intGraph, layout=l)
dev.off()
...
geneVennPlot <- ggvenn(vennData,set_name_size = 4,auto_scale = F) + 
...
# Plot the distribution of degrees for the two datasets
...
degreePlot <- ggplot(degreeData, aes(x = degree, fill = dataset)) +
                  ...
                  scale_x_log10()
# Do a t-test to see if the means are different on the log scale

ttestDegree <- t.test(log(degreeData$degree[degreeData$dataset == "OT Genetics (L2G > 0.4)"]),
                      log(degreeData$degree[degreeData$dataset == "EVA/Orph (Score > 0.5)"]))



# Convert the means back to the original scale

exp(mean(log(degreeData$degree[degreeData$dataset == "OT Genetics (L2G > 0.4)"])))
exp(mean(log(degreeData$degree[degreeData$dataset == "EVA/Orph (Score > 0.5)"])))
...
firstPlotRareDisease <- cowplot::plot_grid(a,aCountsPlot,ncol = 1,rel_heights = c(3,1))
...
geneLengthPlot <- ggplot() + 
                ...
                labs(title = "Distribution of Gene Lengths",
                      subtitle = paste0("P-value for difference in means (log-transformed): ",format(ttestGeneLength$p.value,digits=4))) + 
                xlab("Gene Length (bp)") + 
                ylab("Density") + 
                theme_classic() +
                guides(fill=guide_legend(title="Genes in:")) +
...
proteinAbundancePlot <- ggplot() + 
                ...
                labs(title = "Distribution of Protein Abundance",
                      subtitle = paste0("P-value (negative binomal glm): ",format(pValue,digits=4))) + 
...
depmapAndProteinPlots <- cowplot::plot_grid(depMapPlot,geneLengthPlot,proteinAbundancePlot,ncol = 3,rel_widths = c(3,3,3))

# Same plot but remove all the legends and plot one legend that is the same for all plots using cowplot::get_legend()

depmapAndProteinPlots <- cowplot::plot_grid(depMapPlot + theme(legend.position = "none"),
                                            geneLengthPlot + theme(legend.position = "none"),
                                            proteinAbundancePlot + theme(legend.position = "none"),
                                            cowplot::get_legend(depMapPlot),ncol = 4,rel_widths = c(3,3,3,0.5))

cowplot::ggsave2("depmapAndProteinPlots.png",depmapAndProteinPlots,width = 16,height = 6)
...
vennData <- list("a" = unique(rareDiseaseAssocsFromOTgenetics$targetId),
                "b" = unique(assocDataRareDiseasesCollapsed$targetId),
                "c" = unique(mouseAssocs$targetId))
                  
names(vennData) <- c(paste0("OT Genetics (L2G > ",scoreCutoffOT,")"),paste0("EVA/Orph (Score > ",scoreCutoffOrphanet,")"),"mouse")
...
# Get the distribution of edges in the interaction graph intGraph that are in the two datasets
