...
pathToCSV <- "/home/gummi/netprop/results/AUROCtestNew"
...
DFs <- lapply(DFs, function(x) x %>% filter(nseeds > 25) %>% select(mean_0.25,mean_0.75,mean_0.9,trait,nseeds))

DF <- do.call(rbind,DFs)

DF$source <- rep(names(DFs),sapply(DFs,nrow))

DF$source <- factor(DF$source, levels = c("overallPhenos_aurocResults_binarized_noNorm",
                                          "overallPhenos_aurocResults_binarized_ECnormLog",
                                           "overallPhenos_aurocResults_binarized_AverageVecLOR",
                                           "overallPhenos_aurocResults_binarized_permuteNormDegree"),
...
DF <- DF %>% filter(source != "Average NP LOR")

# Plot the density of the mean values

length(unique(DF$trait))

a <-ggplot(DF, aes(x = mean_0.25, fill = source, color = source)) + 
            ...
            xlab("AUROC") +
            ylab("Density") + 
            guides(fill=guide_legend(title="Normalization Method")) +
...
b <- ggplot(DF, aes(x = source, y = mean_0.25, fill = source)) + 
                    geom_boxplot() + 
                    theme_classic() + 
                    ...
                    ggpubr::stat_compare_means(comparisons = list(c("No Normalization","EC Normalization"),
                                                                 c("No Normalization","Permutation Normalization"),
                                                                 c("EC Normalization","Permutation Normalization")),
                                               method = "t.test") +
                    ggsci::scale_fill_npg() + 
...
# Plot a scatter plot of EC norm vs permute norm

# Pivot wider

DFpivot <- tidyr::pivot_wider(DF[,c("trait","mean_0.25","source")], names_from = source, values_from = mean_0.25)

# Do a scatter plot

c <- ggplot(DFpivot, aes(x = `EC Normalization`, y = `Permutation Normalization`)) + geom_point() + geom_smooth(method = "lm") + theme_classic() + xlab("EC Normalization") + ylab("Permutation Normalization")
d <- ggplot(DFpivot, aes(x = `EC Normalization`, y = `No Normalization`)) + geom_point() + geom_smooth(method = "lm") + theme_classic() + xlab("EC Normalization") + ylab("No Normalization")
e <- ggplot(DFpivot, aes(x = `No Normalization`, y = `Permutation Normalization`)) + geom_point() + geom_smooth(method = "lm") + theme_classic() + xlab("No Normalization") + ylab("Permutation Normalization")

combPlotScatter <- cowplot::plot_grid(c,d,e, rel_widths = c(1,1,1))

combPlot <- cowplot::plot_grid(a,b, rel_widths = c(1,1.5))
combPlot
# Save the plot as a pdf
ggsave("AverageAUROC.png",combPlot, width = 10, height = 4)

aov(mean_0.25 ~ source, data = DF) %>% summary()

# Do a linear model

lm(mean_0.25 ~ source, data = DF) %>% summary()
