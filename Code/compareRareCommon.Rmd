...
assocDataRareDiseases <- assocDataBySource %>% filter(datasourceId == "orphanet")



res <- runNetProp(network = intGraph,
                    assocData = assocDataRareDiseases,
...
testRes <- compareDistanceMetric(as.matrix(resDF),
                      statDistFunctionWrapper,
                      list("method" = "manhattan"),
...
#plot(testRes[["DensityAll"]])
#lines(testRes[["densityEstRand"]], col = "blue")
#lines(testRes[["densityEstRel"]] , col = "red")
...
settingsPerm1 <- list("nSamples" = 200, "perserveDegree" = FALSE, "degreeSampleSmoothing" = 0, "minBucketSize" = 1)
