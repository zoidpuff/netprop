...
# Remove rows with na in nseeds in test

$test <- test %>% filter(!is.na(nseeds))

# Plot the relationship between the number of seed genes and the mean_0.75 

#ggplot(test, aes(x = nseeds, y = mean_0.9)) + geom_point() + geom_smooth(method = "lm") + theme_bw() + ggtitle("Scatter plot of AUROC values for Drop_0.75") + scale_x_log10()
...
DFs <- lapply(DFs, function(x) x %>% filter(nseeds > 30) %>% select(mean_0.25))
...
a <-ggplot(DF, aes(x = mean_0.25, fill = source)) + 
...
b <- ggplot(DF, aes(x = source, y = mean_0.25, fill = source)) + 
                    ...
                    ggpubr::stat_compare_means(comparisons = list(c("No Normalization","Average NP LOR"),
                                                                 c("No Normalization","Eigen Centrality Normalization"),
                                                                 c("Average NP LOR","Eigen Centrality Normalization")),
